-- Enhanced Lost and Found Platform Database Schema
-- This migration adds the missing tables and enhancements for the Lost and Found platform
...
-- Enhanced Lost & Found Reports Table
...
-- User Reputation and Achievements Table
...
CREATE TABLE public.device_matches (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX idx_lost_found_type_status ON public.lost_found_reports (report_type, verification_status);

CREATE INDEX idx_community_tips_location ON public.community_tips 
USING GIST (ll_to_earth(tip_location_lat, tip_location_lng));

CREATE INDEX idx_community_tips_report ON public.community_tips (report_id, created_at DESC);

CREATE INDEX idx_device_matches_status ON public.device_matches (status, created_at DESC);

CREATE INDEX idx_community_events_location ON public.community_events 
USING GIST (ll_to_earth(location_lat, location_lng));

CREATE INDEX idx_community_events_date ON public.community_events (start_date, status);
...
BEGIN
    IF NEW.status = 'recovered' AND OLD.status != 'recovered' THEN
        ...
        VALUES (NEW.found_report_id, 1, 10)
        ON CONFLICT (user_id) 
        DO UPDATE SET 
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.lost_found_reports TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.user_reputation TO authenticated;
...
-- Create RLS policies
CREATE POLICY "Users can view their own reports" ON public.lost_found_reports
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can create their own reports" ON public.lost_found_reports
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own reports" ON public.lost_found_reports
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can view public reports" ON public.lost_found_reports
    FOR SELECT USING (verification_status != 'rejected');

CREATE POLICY "Users can view their own reputation" ON public.user_reputation
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can view public reputation" ON public.user_reputation
    FOR SELECT USING (trust_level IN ('verified', 'trusted', 'expert'));
