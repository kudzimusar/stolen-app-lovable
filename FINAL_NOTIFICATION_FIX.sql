...
-- This handles the actual table structure with feature_category column

-- Step 1: Check the actual table structure and fix it
DO $$
BEGIN
    -- Check if the table has feature_category column (which seems to be the case)
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'notification_preferences' 
        AND column_name = 'feature_category'
    ) THEN
        -- The table uses feature_category instead of category
        -- Let's update the existing records to have proper feature_category values
        UPDATE notification_preferences 
        SET feature_category = 'admin' 
        ...
        IF NOT EXISTS (
            ...
            ADD COLUMN category VARCHAR(30) DEFAULT 'admin';
        END IF;
        
        -- Copy feature_category to category for consistency
        UPDATE notification_preferences 
        SET category = feature_category 
...
-- Step 9: Insert notification preferences for admin users (using the correct column names)
INSERT INTO notification_preferences (user_id, feature_category, email_enabled, push_enabled, sms_enabled, in_app_enabled)
SELECT 
  u.id,
  'admin',
  true,
...
ON CONFLICT (user_id, feature_category) DO UPDATE SET
  email_enabled = true,
  push_enabled = true,
  sms_enabled = false,
  in_app_enabled = true;
...
CREATE INDEX IF NOT EXISTS idx_notification_preferences_feature_category ON notification_preferences(feature_category);
CREATE INDEX IF NOT EXISTS idx_notification_preferences_user_feature_category ON notification_preferences(user_id, feature_category);
...
SELECT 
  'Final notification system setup complete!' as status,
