...
import { Bell, X, CheckCircle, AlertTriangle, Clock, Smartphone } from 'lucide-react'
...
export const DeviceNotificationCenter = () => {
  const { user } = useAuth()
  ...
  const fetchNotifications = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select('*')
        .eq('user_id', user.id)
        .eq('feature_category', 'device_management')
  ...
  useEffect(() => {
    ...
    // Set up real-time subscription for device notifications
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          const newNotification = payload.new as DeviceNotification
          if (newNotification.feature_category === 'device_management') {
