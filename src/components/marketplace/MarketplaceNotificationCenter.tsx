...
import { Bell, X, CheckCircle, AlertTriangle, Clock, ShoppingBag, DollarSign } from 'lucide-react'
...
export const MarketplaceNotificationCenter = () => {
  const { user } = useAuth()
  ...
  const fetchNotifications = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select('*')
        .eq('user_id', user.id)
        .eq('feature_category', 'marketplace')
  ...
  useEffect(() => {
    ...
    // Set up real-time subscription for marketplace notifications
    const channel = supabase
      .channel('marketplace_notifications')
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
        ...
        (payload) => {
          const newNotification = payload.new as MarketplaceNotification
          if (newNotification.feature_category === 'marketplace') {
        ...
        <Card className="absolute right-0 top-12 w-80 z-50 shadow-lg">
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm font-medium">Marketplace Notifications</CardTitle>
