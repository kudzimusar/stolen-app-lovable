...
export const PhotoUpload: React.FC<PhotoUploadProps> = ({
  ...
  const openCamera = useCallback(async () => {
    if (!enableCamera) {
      toast({
        ...
        description: "Camera feature is disabled for this upload",
        variant: "destructive"
      });
      return;
    }
  ...
  }, [handleFileSelect, toast, enableCamera]);
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      ...
      <div 
        className={`relative p-4 border-2 border-dashed rounded-xl bg-gradient-to-br transition-all ${
          dragActive 
            ? 'border-primary bg-primary/10' 
            : 'border-primary/20 bg-primary/5 hover:border-primary/40'
        } ${enableDragDrop ? 'cursor-pointer' : ''}`}
        onDragEnter={enableDragDrop ? handleDrag : undefined}
        onDragLeave={enableDragDrop ? handleDrag : undefined}
        onDragOver={enableDragDrop ? handleDrag : undefined}
        onDrop={enableDragDrop ? handleDrop : undefined}
        onClick={enableDragDrop ? () => fileInputRef.current?.click() : undefined}
      >
        <div className="space-y-3">
          {uploading ? (
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground text-center font-medium">
                {ocrProcessing ? 'Processing with OCR...' : 'Uploading...'} {uploadProgress}%
              </p>
            </div>
          ) : (
            <>
          ...
          <div className="flex justify-center gap-2 text-xs text-muted-foreground/80 pt-1 flex-wrap">
