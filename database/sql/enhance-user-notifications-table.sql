-- Enhance existing user_notifications table to support all 18 features
-- This preserves the working Lost & Found system while adding feature categorization

-- STEP 1: Add feature categorization columns FIRST
...
-- STEP 3: Update existing Lost & Found notifications to have proper feature_category
-- Only run this AFTER the column has been created
...
-- RLS policies for notification_preferences (with IF NOT EXISTS)
DO $$
BEGIN
...
DO $$
BEGIN
    DROP POLICY IF EXISTS "Authenticated users can view delivery logs" ON notification_delivery_logs;
