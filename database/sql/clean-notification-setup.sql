...
-- This script handles all potential conflicts and can be run multiple times safely

-- STEP 1: Add columns to user_notifications (safe)
...
-- STEP 3: Drop and recreate tables to avoid conflicts
...
-- STEP 6: Create RLS policies (clean approach)
...
BEGIN
    ...
    FOR policy_name IN 
        SELECT policyname FROM pg_policies WHERE tablename = 'notification_preferences'
    LOOP
        EXECUTE format('DROP POLICY IF EXISTS %I ON notification_preferences', policy_name);
    END LOOP;
    ...
    LOOP
        EXECUTE format('DROP POLICY IF EXISTS %I ON email_templates', policy_name);
    END LOOP;
    
    -- Drop all policies on notification_delivery_logs
    FOR policy_name IN 
        SELECT policyname FROM pg_policies WHERE tablename = 'notification_delivery_logs'
    LOOP
        EXECUTE format('DROP POLICY IF EXISTS %I ON notification_delivery_logs', policy_name);
    END LOOP;
END $$;
...
-- STEP 8: Insert email templates for all 18 features
...
SELECT 'CLEAN NOTIFICATION SYSTEM SETUP COMPLETE!' as status,
