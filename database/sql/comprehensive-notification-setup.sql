...
-- Addresses "column does not exist" error with proper transaction handling
-- This script sets up the complete database infrastructure for all 18 notification features

BEGIN;

-- SECTION 1: Add Columns to user_notifications
DO $$
BEGIN
...
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Column addition error (may already exist): %', SQLERRM;
END $$;

-- SECTION 2: Verify Columns Exist Before Proceeding
DO $$
BEGIN
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'feature_category column was not created. Aborting.';
    END IF;
    RAISE NOTICE 'Column verification passed';
END $$;

-- SECTION 3: Create Indexes (only after columns verified)
DO $$
BEGIN
...
-- SECTION 4: Create notification_preferences Table
...
-- SECTION 5: Create email_templates Table
...
-- SECTION 6: Create notification_delivery_logs Table
...
-- SECTION 7: Create Indexes for New Tables
...
-- SECTION 8: Insert Email Templates for All 18 Features
